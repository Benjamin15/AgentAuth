...
from adapters import MockAdapter, GeminiAdapter
import datetime
...
async def proxy_request(integration_name: str, request: Request, authorization: str = Header(None), db: Session = Depends(get_db)):
    ...
    if agent.is_frozen:
        log = AuditLog(agent_id=agent.id, target_service=integration_name, request_details="Blocked by Kill Switch", response_status=403)
        db.add(log)
        db.commit()
        raise HTTPException(status_code=403, detail="Agent is currently frozen (Kill Switch Enabled).")

    # 2. IAM Check (Scoped Permissions)
    permission = db.query(AgentPermission).filter(
        AgentPermission.agent_id == agent.id, 
        AgentPermission.scope == integration_name
    ).first()
    
    if not permission:
        log = AuditLog(agent_id=agent.id, target_service=integration_name, request_details="Unauthorized Scope Access", response_status=403)
        db.add(log)
        db.commit()
        raise HTTPException(status_code=403, detail=f"Permission denied: Agent does not have the '{integration_name}' scope.")

    # 3. Check Integration and Get Adapter
    integration = db.query(Integration).filter(Integration.name == integration_name).first()
    if not integration:
        raise HTTPException(status_code=400, detail=f"Integration '{integration_name}' not found in database")
    
    if integration_name == "gemini":
        if not integration.provider_key:
            raise HTTPException(status_code=500, detail="Gemini API Key not configured in AgentAuth")
        adapter = GeminiAdapter(api_key=integration.provider_key)
    elif integration_name == "mock":
        adapter = MockAdapter()
    else:
        raise HTTPException(status_code=400, detail=f"Adapter for '{integration_name}' not implemented")
    ...
    log = AuditLog(agent_id=agent.id, target_service=integration_name, request_details=str(body), response_status=status_code)
    db.add(log)
    db.commit()
...
def create_agent(payload: dict, db: Session = Depends(get_db)):
    ...
    return {"status": "success", "agent": {"id": new_agent.id, "name": new_agent.name, "api_key": new_agent.api_key}}
